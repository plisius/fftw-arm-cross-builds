...
# =============================================== #
# FFTW Cross-Compilation Script for ARMv8-A       #
# (Hard/Soft float with automated setup)          #
# =============================================== #

# Автоматизированная сборка библиотеки FFTW и бенчмарка
# - Режимы: hard float (NEON) и soft float (ARMv7 совместимость)
...
echo "=== [FFTW Cross Compilation Universal Script: ARMv8-A] ==="

# --- 1. Режим сборки ---

echo "Выберите режим сборки:"
echo "1) ARMv8-A 64-bit HARD float (aarch64-linux-gnu + NEON, Cortex-A53/A55/A76 и др.)"
...
if [[ "$mode" == "1" ]]; then
    ...
    BINARY_NAME="fftw_bench_armv8a_hard_float"
    REQUIRED_PKGS=("gcc-aarch64-linux-gnu" "g++-aarch64-linux-gnu")
elif [[ "$mode" == "2" ]]; then
    TARGET_TRIPLET="arm-linux-gnueabi"
    CFLAGS="-O3 -march=armv7-a -mfloat-abi=soft"
    ENABLE_NEON="--disable-neon"
    FFTW_PREFIX="/usr/${TARGET_TRIPLET}"
    BINARY_NAME="fftw_bench_armv8a_soft_float"
